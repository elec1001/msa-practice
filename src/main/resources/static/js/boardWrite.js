...
$(document).ready(()=>{
    checkToken();
    setupAjax();
    //사용자 정보를 가져오고 성공하면 input태그에 사용자 이름과 아이디를 저장
...
let updateFileList=()=>{
    $('#fileList').empty();//기존 목록 지우기

    if(selectedFile){
        $('#fileList').append(`<li>${selectedFile.name}<button type="button" class="remove-btn">X</button></li>`);
        //X버튼 클릭 시 파일 제거
